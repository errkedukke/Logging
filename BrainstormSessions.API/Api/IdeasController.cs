...
namespace BrainstormSessions.Api
{
    public class IdeasController : ControllerBase
    {
        ...
        public async Task<ActionResult<BrainstormSession>> CreateActionResult([FromBody] NewIdeaModel model)
        {
            if (!ModelState.IsValid)
            {
                _logger.LogError("ModelState is invalid. Errors: {ModelStateErrors}",
            ...
            if (session == null)
            {
                _logger.LogError("Brainstorm session with ID {SessionId} not found.", model.SessionId);
                return NotFound(model.SessionId);
            }
            ...
            _logger.LogInformation("Idea successfully added to session {SessionId}.", model.SessionId);
